...
export default function OnboardingModal({
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(formatAuthError(err, "Sign in error. Please try again."));
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const renderErrorBox = () => {
    ...
    const isBrowserNotice =
      error.toLowerCase().includes("chrome") ||
      error.toLowerCase().includes("safari") ||
      error.toLowerCase().includes("browser") ||
      error.toLowerCase().includes("discord") ||
      isInApp;

    return (
      <div className="p-3 rounded-xl bg-rose-950/40 border border-rose-800/60 text-rose-300 text-xs font-mono flex flex-col gap-2.5">
        ...
        {isBrowserNotice && (
          <div className="pt-2 border-t border-rose-800/40 flex flex-wrap items-center justify-between gap-2">
            <button
              type="button"
              onClick={() => {
                const url = typeof window !== "undefined" ? window.location.origin : "https://www.getreax.com";
              ...
              className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-[11px] font-mono rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer"
            >
              {copiedLink ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
              <span>{copiedLink ? "Link copied!" : "Copy link for Chrome/Safari"}</span>
            </button>

            <a
              href={typeof window !== "undefined" ? window.location.origin : "https://www.getreax.com"}
              target="_blank"
        ...
        <div className="p-6 space-y-4">
          ...
          {needsEmailConfirm ? (
            ...
            <>
              ...
              {tab === "signin" && isForgotPassword ? (
                <form onSubmit={handleForgotPassword} className="space-y-3.5">
                  <div className="space-y-1">
                    <h3 className="text-sm font-bold text-white font-sans">Reset Password</h3>
                  ...
                  <button
                    ...
                    className="w-full py-2 text-center text-xs font-mono text-slate-400 hover:text-white transition-colors cursor-pointer"
                  >
                    ← Back to Sign In
                  </button>
                </form>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <button
                        ...
                        className="text-[11px] font-mono text-indigo-400 hover:text-indigo-300 transition-colors cursor-pointer hover:underline"
                      >
                        Forgot password?
                      </button>
                    </div>
              ...
              {tab === "signup" && (
                <form onSubmit={handleSignUp} className="space-y-3">
                  {isInApp && (
                    <div className="p-2.5 rounded-xl bg-indigo-950/50 border border-indigo-700/60 text-indigo-200 text-xs font-mono flex items-start gap-2.5 shadow-sm">
                      <Compass className="w-4 h-4 text-indigo-400 shrink-0 mt-0.5" />
                      <div className="leading-snug">
                        <span className="font-semibold text-white">Opening from Discord?</span> For the smoothest sign-up, tap <span className="text-white font-bold">⋮</span> in the top right and choose <span className="text-indigo-300 font-semibold">"Open in Chrome"</span> or <span className="text-indigo-300 font-semibold">"Open in Safari"</span>.
                      </div>
                    </div>
                  )}
